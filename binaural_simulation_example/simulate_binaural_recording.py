...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--source",
        type=Path,
        default="cmu_arctic_us_axb_a0004.wav",
        help="Path to speech or other source audio file",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default="simulate_binaural_recording.wav",
    ...
    e_absorption, max_order = pra.inverse_sabine(0.3, room_dim)
    room = pra.ShoeBox(
        room_dim,
        fs=fs,
        max_order=40,
        # materials=pra.Material(0.5),
        materials=pra.Material(e_absorption), #all_materials,
        air_absorption=True,
        ray_tracing=False,
        min_phase=False,
        use_rand_ism=True,
    ...
    for idx, fb in enumerate(range(44)):
        ...
        # ax.set_yticks([])
        # ax.set_zticks([])
        ax.set_title((idx+1)*fs/512, fontsize=8)

    fig.tight_layout()  # Auto-adjust subplots to fit in figure
    plt.show()
